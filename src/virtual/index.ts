...
export function updateVirtualModule(
  plugin: Plugin,
  id: string,
  value: VirtualModule
): void {
  ...
  const plugin = {
    ...
    resolveId(id: string, importer: string) {
      ...
      if (importer) {
        ...
        if (resolvedIds.has(resolved)) return VIRTUAL_PREFIX + resolved;
      }
      return null;
    },
    load(id: string) {
      if (id.startsWith(VIRTUAL_PREFIX)) {
        ...
        const resolvedId =
          idNoPrefix in modules ? idNoPrefix : resolvedIds.get(idNoPrefix);
        if (resolvedId) {
